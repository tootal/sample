...
const std::vector<std::string> Data::coding_schedule = {
    ...
    "stop",    "then",      "to",      "true",    "until", "var",    "while",
    "write",   IDENTIFIER,  INTEGER,   STRING,    "(",     ")",      "*",
    "*/",      "+",         ",",       "-",       ".",     "..",     "/",
    "/*",      ":",         ":=",      ";",       "<",     "<=",     "<>",
    "=",       ">",         ">=",      "[",       "]"};
...
const std::vector<std::string> Data::boolean = {"true", "false"};

const std::vector<std::string> Data::type = {"integer", "bool", "char"};

unsigned Data::getCode(const std::string &word) {
    iter pos =
        std::find(coding_schedule.cbegin(), coding_schedule.cend(), word);
